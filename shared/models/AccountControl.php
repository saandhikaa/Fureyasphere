<?php
    class AccountControl {
        ...
        public function userRegistered ($username) {
            $result = $this->db->fetching("SELECT * FROM $this->table WHERE username_ = ?", [$username]);
            return !empty($result);
        }
        ...
        public function signUp ($data, $level = 3) {
            ...
            $query = "INSERT INTO $this->table (time_, username_, password_, level_, agreed_) VALUES (?, ?, ?, ?, ?)";
            $params = [$time, $data["username"], $hashedPassword, $level, $agreement_accepted];
            return $this->db->executing($query, $params);
        }
